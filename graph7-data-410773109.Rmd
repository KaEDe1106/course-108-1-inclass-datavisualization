...
##取出一週大事中的所有國家，手動整理出符合要求的`identities`
...
##在世界地圖中標示df_politics_news中，有出現的各個國家
...
##因世界地圖用的是小比例尺，涵蓋範圍過大，不方便我們標上label，於是各洲分別畫一張比例尺較大的地圖，到時候在上面畫label
world %>%
  filter(
    continent == "Africa"
  ) %>%
...
library(osmdata)
#特別處理香港部分(原world資料中並未將香港的資料獨立成一欄)
